...
const servicesData = {
	title: 'Services',
	main: [
		{ title: 'CRM Solutions', desc: 'Manage customer relationships', icon: Users },
		{ title: 'ERP Systems', desc: 'Enterprise resource planning', icon: Layers },
		{ title: 'Web Development', desc: 'Modern and fast web apps', icon: Globe },
	],
	side: [
		{ title: 'Mobile App Dev', href: '#', icon: Smartphone },
		{ title: 'AI Chatbots', href: '#', icon: Bot },
		{ title: 'Business Automation', href: '#', icon: Zap },
	]
};
...
const companyData = {
	title: 'Company',
	main: [
		{ title: 'About Us', desc: 'Our mission and vision', icon: Info },
		{ title: 'Blogs', desc: 'Latest news and articles', icon: BookOpen },
		{ title: 'Careers', desc: 'Join our growing team', icon: Briefcase },
	],
	side: []
};
		...
		<>
			...
			<header className="sticky top-0 z-50 w-full border-b bg-white/80 backdrop-blur-md supports-[backdrop-filter]:bg-white/60">
				...
				<div className="flex items-center gap-4">
					<AnimatedThemeToggler className="hidden sm:flex size-12 items-center justify-center rounded-full bg-slate-100/80 text-slate-600 hover:bg-slate-200 hover:text-blue-600 transition-colors cursor-pointer border border-slate-200" />
...
function DropdownMenu({ data, pathname }: { data: typeof servicesData, pathname: string }) {
	...
	return (
		<div 
			...
			onMouseLeave={() => setIsHovered(false)}
		>
			<div className="flex items-center gap-1 cursor-pointer text-lg font-medium text-slate-600 hover:text-blue-600 transition-colors">
				{data.title}
				<motion.div
			...
			<AnimatePresence>
				{isHovered && (
					...
					>
						<div className="bg-white rounded-3xl shadow-2xl shadow-slate-200/50 border border-slate-100 p-6 flex gap-6">
					...
					<div className="flex-1 grid grid-cols-3 gap-4">
						{data.main.map((item, idx) => (
							<Link 
								key={idx} 
								href="#" 
								className="relative flex flex-col justify-between p-5 rounded-2xl border border-slate-100 bg-white hover:border-blue-200 hover:bg-blue-50/10 transition-all group/card overflow-hidden min-h-[220px]"
							>
								...
								<div className="relative z-10 text-slate-700 group-hover/card:text-blue-600 transition-colors">
									<item.icon className="size-7 stroke-[1.5]" />
								</div>
								<div className="relative z-10 mt-8">
									<h4 className="font-bold text-slate-900 text-lg group-hover/card:text-blue-700 transition-colors">{item.title}</h4>
					...
					{data.side && data.side.length > 0 && (
						<div className="w-64 flex flex-col justify-center border-l pl-6 gap-1">
	...
	return (
		<Sheet>
			<SheetTrigger render={<Button size="icon" variant="ghost" className="rounded-full lg:hidden" />}>
				<MenuIcon className="size-7 text-slate-700" />
			</SheetTrigger>
			<SheetContent
				className="bg-white/95 supports-[backdrop-filter]:bg-white/80 w-full md:max-w-md backdrop-blur-xl border-l p-0 flex flex-col"
				showCloseButton={false}
			>
				<div className="flex h-24 items-center justify-between border-b px-8">
					<p className="font-sans text-3xl font-extrabold tracking-tight text-slate-900">
						Indux<span className="text-blue-600">.</span>
					</p>
					<SheetClose render={<Button size="icon" variant="ghost" className="rounded-full" />}>
						<XIcon className="size-7 text-slate-700" />
				...
				<div className="flex-1 overflow-y-auto py-8 px-8 flex flex-col gap-6">
					...
					<div>
						<button 
							...
							className="flex items-center justify-between w-full text-2xl font-bold text-slate-900 mb-2"
						>
							Services
							<ChevronDown className={cn("size-6 transition-transform", openSection === 'services' && "rotate-180")} />
						</button>
						{openSection === 'services' && (
							<div className="flex flex-col gap-4 mt-4 ml-4 border-l-2 pl-4">
								{servicesData.main.map((item) => (
									<Link key={item.title} href="#" className="text-lg font-medium text-slate-600 hover:text-slate-900">{item.title}</Link>
								))}
								{servicesData.side.map((item) => (
									<Link key={item.title} href={item.href} className="text-lg font-medium text-slate-600 hover:text-slate-900">{item.title}</Link>
								))}
							</div>
						)}
					</div>
					...
					<SheetClose render={<Link href="/products" className="block text-2xl font-bold text-slate-900 hover:text-slate-600 transition-colors" />}>
						Products
					</SheetClose>
					<SheetClose render={<Link href="/contact" className="block text-2xl font-bold text-slate-900 hover:text-slate-600 transition-colors" />}>
						Contact Us
					</SheetClose>
				</div>
