...
import { ArrowLeft, Calendar, User, Clock, Tag, Share2, Loader2, Bookmark } from 'lucide-react';
  ...
  if (loading) {
    return (
      <div className="flex flex-col min-h-screen bg-slate-50 dark:bg-slate-950 justify-center items-center">
        ...
        <span className="text-slate-500 text-sm mt-3 font-mono">// Loading Article Content...</span>
      </div>
    );
  }
  ...
  if (!blog) {
    return (
      <div className="flex flex-col min-h-screen bg-slate-50 dark:bg-slate-950 justify-center items-center p-6 text-center">
        <Bookmark className="w-16 h-16 text-slate-350 dark:text-slate-700 mb-4" />
        <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Article not found</h2>
        <p className="text-slate-500 text-sm mt-2 max-w-sm">The article you are looking for does not exist or has been removed.</p>
        <button
          onClick={() => router.push('/blogs')}
          className="mt-6 px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-full text-sm font-semibold transition-all cursor-pointer"
        >
          Back to Blog Feed
        </button>
      </div>
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-50 dark:bg-slate-950 transition-colors duration-300">
      <main className="flex-grow max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12 relative z-10">
        ...
        <button
          onClick={() => router.push('/blogs')}
          className="inline-flex items-center gap-2 text-sm font-medium text-slate-550 hover:text-blue-600 dark:text-slate-400 dark:hover:text-blue-400 mb-8 transition-colors group cursor-pointer"
        >
        ...
        <article className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-8 flex flex-col gap-6">
            ...
            >
              ...
              {/* Title */}
              <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold tracking-tight text-slate-900 dark:text-white leading-[1.15] mb-6">
                {blog.title}
              </h1>
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-450 dark:text-slate-500">Indux Contributor</p>
                  </div>
                </div>
              </div>
